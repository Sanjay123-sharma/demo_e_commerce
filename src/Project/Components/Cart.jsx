...
export default function Cart() {
  ...
  <div className="min-h-screen flex flex-col bg-gray-100">
      ...
      <main className="flex-1 container mx-auto mt-24 px-4 flex gap-6">
        {Cart.length === 0 ? (
          <h1 className="text-center text-red-600 text-xl w-full">Cart is Empty</h1>
        ) : (
          <>
            {/* Cart Items Grid */}
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 w-full">
              {Cart.map((item) => (
                <div key={item.id} className="bg-white shadow rounded p-3 text-sm h-[300px] flex flex-col justify-between">
                  <h2 className="font-medium truncate">{item.title}</h2>
                  <p className="text-yellow-600 text-xs">Rating: {item.rating}</p>
